...
export function CarForm({
  ...
  const [existingPhotosState, setExistingPhotosState] =
  ...
  const {
    ...
    defaultValues: {
      ...
      deposit_amount: normalizeOptionalFeeDefault(
        defaultValues?.deposit_amount,
      ),
      outside_accra_fee: normalizeOptionalFeeDefault(
        defaultValues?.outside_accra_fee,
      ),
    },
  });
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const computedTitle = buildListingTitle(
      values.brand,
      values.model,
      values.car_year,
    );
    ...
    try {
      ...
      const res = await fetch(
        targetCarId ? `/api/cars/${targetCarId}` : "/api/cars",
        {
  ...
  const remainingSlots = Math.max(
    MAX_PHOTOS - existingPhotosState.length - files.length,
    0,
  );
  const autoTitlePreview = buildListingTitlePreview(
    brandValue,
    modelValue,
    yearValue,
  );
  ...
  const handleFileSelection = (selected: File[]) => {
    ...
    if (room <= 0) {
      setPhotoWarning(
        `You already have ${MAX_PHOTOS} photos. Remove one before adding another.`,
      );
      return;
    }

    const oversized = selected.filter(
      (file) => file.size > MAX_PHOTO_FILE_BYTES,
    );
    const allowedBySize = selected.filter(
      (file) => file.size <= MAX_PHOTO_FILE_BYTES,
    );
    ...
    if (oversized.length > 0) {
      const largestMb =
        Math.max(...oversized.map((file) => file.size)) / (1024 * 1024);
      setPhotoWarning(
      ...
      setPhotoWarning(
        `Only ${MAX_PHOTOS} photos are allowed. Extra files were ignored.`,
      );
    } else {
      setPhotoWarning(null);
    }
  ...
  const replaceExistingPhoto = async (photoId: string, file: File) => {
    ...
    if (file.size > MAX_PHOTO_FILE_BYTES) {
      setPhotoWarning(
        "Photo is too large to upload. Please use an image under 4MB.",
      );
      return;
    }
    setPhotoMutatingId(photoId);
    ...
    try {
      ...
      const payload = (await parseApiResponsePayload(res)) as {
        message?: string;
        data?: ExistingPhoto;
      };
      if (!res.ok) {
        throw new Error(
          payload.message ??
            mapUploadErrorMessage(res.status, "Unable to replace photo"),
        );
      }
      if (!payload.data?.url) {
      ...
      setExistingPhotosState((prev) =>
        prev.map((photo) =>
          photo.id === photoId
            ? { id: photo.id, url: payload.data!.url }
            : photo,
        ),
      );
    } catch (err: unknown) {
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-600">
            Generated automatically from brand, model and year.
          </p>
        </div>
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-700">
        ...
        <div className="space-y-2">
          ...
          {carCatalogError ? (
            <p className="text-xs text-red-600">
              Car catalog failed to load. Refresh and try again.
            </p>
          ) : null}
        </div>
        <div className="space-y-2">
          ...
          >
            ...
            {(
              makes.find((make) => make.name === watch("brand"))?.models ?? []
            ).map((model) => (
        ...
        <div className="space-y-2">
          ...
          />
          {errors.car_year?.message ? (
            <p className="text-xs text-red-600">{errors.car_year.message}</p>
          ) : null}
        </div>
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-700">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between rounded-md border border-border bg-white px-3 py-2">
            <p className="text-xs text-gray-600">
              Enable this first to set a delivery charge.
            </p>
        ...
        <div className="space-y-2">
          ...
          {!deliveryAvailable ? (
            <p className="text-xs text-gray-500">
              Leave blank when delivery is off.
            </p>
          ) : null}
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            min={0}
            placeholder="0"
            {...register("insurance_fee", optionalNumberField)}
          />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-700">
          ...
          <Input
            ...
            min={0}
            placeholder="0"
            {...register("deposit_amount", optionalNumberField)}
          />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-700">
            Outside listing region surcharge (GHS)
          </label>
          <Input
            type="number"
            min={0}
            placeholder="0"
            {...register("outside_accra_fee", optionalNumberField)}
          />
          <p className="text-xs text-gray-500">
            Added automatically when the renter selects a trip use location
            outside your listing region.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea
          rows={4}
          placeholder="Describe your car, pickup and rules"
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-gray-600">
          Recommended: upload {MIN_PHOTOS} to {MAX_PHOTOS} photos with clear
          exterior/interior coverage for faster approval.
        </p>
        <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          Photos are not auto-rejected during upload. Listings with low-quality
          photos (for example blurry shots, poor lighting, wrong
          framing/centering, or too few angles) may fail approval.
        </p>
        <p className="text-xs text-gray-600">
          If upload fails on mobile, use photos under 4MB each (phone images are
          sometimes too large).
        </p>
          ...
          <div className="mt-2 space-y-1 text-xs text-gray-700">
            {qualityChecklist.map((item, idx) => {
              ...
              return (
                <p
                  key={item}
                  className={done ? "text-emerald-700" : "text-gray-600"}
                >
            ...
            <p className="text-amber-700">
              Review each photo before publishing and avoid showing a readable
              plate.
            </p>
          </div>
        </div>
        <p className="text-xs font-semibold text-gray-700">
          Photos total: {totalPhotos} / {MIN_PHOTOS} recommended minimum /{" "}
          {MAX_PHOTOS} max
        </p>
        {remainingSlots > 0 ? (
          <p className="text-xs text-gray-600">
            You can add {remainingSlots} more photo(s).
          </p>
        ) : (
        ...
        {existingPhotosState.length > 0 ? (
          <div className="space-y-1">
            <p className="text-xs text-gray-700">
              {existingPhotosState.length} current photo(s)
            </p>
        ...
        {filePreviews.length > 0 ? (
          <div className="space-y-1">
            <p className="text-xs text-gray-700">
              {filePreviews.length} new file(s) selected
            </p>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
      ...
      <div className="flex items-center justify-between rounded-lg border border-border bg-white p-4">
        <div>
          ...
          <p className="text-xs text-gray-600">
            Toggle to disable bookings temporarily.
          </p>
        </div>
      ...
      <div className="flex items-center justify-between rounded-lg border border-border bg-white p-4">
        <div>
          ...
          <p className="text-xs text-gray-600">
            Enable to auto-confirm bookings after payment.
          </p>
        </div>
        <label className="flex items-center gap-2 text-sm">
      ...
      {photoWarning ? (
        <p className="text-sm text-amber-700">{photoWarning}</p>
      ) : null}
      {submitProgressMessage ? (
        <p className="text-sm text-gray-600">{submitProgressMessage}</p>
      ) : null}
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (!payload.data?.id || !payload.data?.url) {
        throw new Error(
          `Upload completed but no photo record was returned for "${file.name}".`,
        );
      }
      currentPhotoCount += 1;
...
function mapUploadErrorMessage(status: number, fallback: string) {
  ...
  if (status === 403)
    return "You do not have permission to upload photos for this car.";
  return fallback;
}
...
function buildPostSubmitRedirectPath(
  basePath: string,
  notice: "submitted" | "updated",
) {
