...
type ApiCar = Database["public"]["Tables"]["cars"]["Row"] & {
...
async function loadCar(id: string): Promise<{
  ...
  const apiBase = (() => {
    ...
    if (headerHost) {
      ...
      return `${isLocal ? "http" : "https"}://${headerHost}`;
    }
  ...
  try {
    ...
    if (res.ok) {
      const { data } = (await res.json()) as { data?: ApiCar };
      if (data) {
        car = mapApiCar(data);
      }
    }
  ...
  // Supabase SSR (for availability + favorites and fallback)
  try {
    ...
    if (!car) {
      const { data: carData } = await supabase
        ...
        .eq("id", id)
        .maybeSingle();
      const supabaseCar = carData as ApiCar | null;
      if (supabaseCar) {
        car = mapApiCar(supabaseCar);
      }
    }
...
function mapApiCar(data: ApiCar): CarDetail {
  return {
    id: data.id,
    title: data.title,
