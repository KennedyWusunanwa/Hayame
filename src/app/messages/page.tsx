...
export default function MessagesPage() {
  const { activeConversationId, closeConversation, loading, openConversation } = useMessaging();
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="grid gap-6 lg:grid-cols-[340px,1fr]">
        <div className={`h-[70vh] rounded-2xl border border-border bg-background shadow-soft ${activeConversationId ? "hidden lg:block" : ""}`}>
          <ConversationList />
        </div>
        <div className={`h-[70vh] ${activeConversationId ? "block" : "hidden lg:block"}`}>
          {activeConversationId ? (
            ...
            <div className="flex h-full items-center justify-center rounded-2xl border border-border bg-background p-8 text-center">
              <div>
                ...
                <p className="mt-2 text-sm text-gray-600">Pick a chat from the inbox to see messages.</p>
              </div>
            </div>
          )}
        </div>
